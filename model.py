...
class AdversarialVAE(nn.Module):
    def __init__(self, weight):
        ...
        self.private_log_var = nn.Linear(
            2 * mconfig.hidden_dim, mconfig.private_hidden_dim)
    ...
    def forward(self, sequences, seq_lengths, age_labels, content_bow, iteration, last_epoch):
        ...
        content_emb_mu, content_emb_log_var = self.get_content_emb(
            sentence_emb)
        private_emb_mu, private_emb_log_var = self.get_private_emb(
            sentence_emb)
        ...
        generative_emb = torch.cat(
            (sampled_private_emb, sampled_content_emb), axis=1)
        ...
        # Discriminator loss
        private_disc_preds = self.get_style_disc_preds(sampled_content_emb)
        private_disc_loss = self.get_style_disc_loss(
            private_disc_preds, age_labels)
        # adversarial entropy
        private_entropy_loss = self.get_entropy_loss(private_disc_preds)
        # Multitask loss
        private_mul_loss = self.get_style_mul_loss(
            sampled_private_emb, age_labels)
        ...
        # ================ total weighted loss ==========#
        vae_and_classifier_loss = mconfig.style_adversary_loss_weight * private_entropy_loss + \
                                  mconfig.style_multitask_loss_weight * private_mul_loss + \
                                  reconstruction_loss + private_kl_loss + content_kl_loss

        return private_disc_loss, vae_and_classifier_loss
