...
class Preprocessor():
    ...
    def preprocess_privacy_text(self):
        print("Privacy Preprocessing started")
        with open(config.train_privacy_text_path, 'w') as text_file, open(config.train_privacy_labels_path,'w') as labels_file:
            with open(config.privacy_text_path, 'r', encoding="iso-8859-1") as reviews_file:
                spamreader = csv.reader(reviews_file, dialect='excel')
                for row in spamreader:
                    line = self._clean_text(row[0])
                    if len(line) > 0:
                        if row[2] == "0":
                            text_file.write(line + "\n")
                            labels_file.write("0" + "\n")
        ...
        print("Privacy Processing complete ")
...
if __name__ == "__main__":
    ...
    preproc.preprocess_privacy_text()